...
- **Test Projects:** Scaffold.Assessment.Tests, Scaffold.Api.Tests, Scaffold.Migration.Tests, Scaffold.Integration.Tests (211+ tests)
- **Coverage Thresholds:** 80% line coverage minimum (overall), 90% for new code, 70% branch coverage, 100% for critical paths (auth, migration engine, data validation)
- **Testing Standards:** See `.github/instructions/testing.instructions.md` for full standards
- **Test Types Required:** Unit (always), Integration (for component interactions), E2E (critical user paths), Regression (every bug fix)
- **Quality Principle:** Tests must verify features **work correctly** — asserting correct values and behavior — not merely that features execute without errors. Coverage percentage alone is insufficient; test quality matters equally.
...
- Define quality gates and acceptance thresholds: 80% line coverage minimum (overall), 90% for new code, 70% branch coverage, 100% for critical paths. Enforce that tests assert on correct behavior, not just absence of errors.
