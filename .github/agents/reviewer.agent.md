...
## Project Knowledge
- **Languages:** C# (.NET 8), TypeScript (React 19)
- **Lint Command:** `cd src/Scaffold.Web && npx eslint .`
- **Coverage Threshold:** 80% line coverage minimum (overall), 90% for new code, 100% for critical paths. Do not approve PRs that reduce overall coverage.
- **PR Merge Criteria:** 1 approving review + CI green; all tests must pass
- **Commit Message Convention:** Conventional Commits: `feat:`, `fix:`, `docs:`, `refactor:`, `test:`, `chore:`, `ci:`
- **Testing Standards:** See `.github/instructions/testing.instructions.md` — when reviewing, verify tests assert on **correct behavior and values**, not just non-null/non-empty. Check that error paths and edge cases are tested, not just happy paths. Frontend tests should verify user-visible behavior, not internal state.
...
- Assess test coverage — are the right things tested? Verify tests check **correct values and behavior**, not just that code doesn't crash. Check that error paths return the right status codes and messages. Verify frontend tests assert on user-visible output, not implementation details. Flag tests that only assert non-null.
