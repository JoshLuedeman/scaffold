...
## Project Knowledge
- **Test Framework:** xUnit + Moq (backend), React Testing Library (frontend when added)
- **Test Command:** `dotnet test` (all backend tests), `cd src/Scaffold.Web && npx tsc --noEmit` (frontend typecheck)
- **Coverage Tool:** XPlat Code Coverage via `dotnet test --collect:"XPlat Code Coverage" --settings coverage.runsettings`
- **Coverage Threshold:** 80% line coverage minimum (overall), 90% for new code, 100% for critical paths (auth, migration, validation). CI should warn below 80%, block below 75%.
- **Test Directory Layout:** `tests/Scaffold.Api.Tests/`, `tests/Scaffold.Assessment.Tests/`, `tests/Scaffold.Migration.Tests/`, `tests/Scaffold.Integration.Tests/`
- **CI Test Command:** `dotnet test --no-build --configuration Release --collect:"XPlat Code Coverage" --settings coverage.runsettings`
- **Testing Standards:** See `.github/instructions/testing.instructions.md` for full coverage requirements, test quality standards, and Scaffold-specific patterns
- **Key Testing Patterns:**
  ...
  - Use `TestAuthHandler` for authenticated test requests, `StubMigrationEngine` for migration engine isolation
  - Tests must verify **correct behavior and values**, not just that something doesn't crash
...
- Every acceptance criterion has at least one corresponding test that verifies the **correct result**, not just that code runs without error
- Happy paths are tested with realistic data — not empty objects or default values
- Edge cases are covered: nulls, empty values, boundaries, invalid inputs, large inputs, single-item and many-item collections
- Error paths are tested: wrong types, missing fields, unauthorized access (401/403), network failures, malformed data — and assertions verify the **correct error type, message, and status code**
- Tests are independent — they can run in any order and still pass
- Test names clearly describe the scenario and expected outcome (e.g., `Assess_WithEmptySchema_ReturnsEmptyReport` not `TestAssess3`)
- Coverage gaps are documented with rationale (not just ignored)
- Regression tests exist for any previously reported bugs, named to reference the bug
- Tests run reliably — no flaky tests that pass sometimes and fail others
- **Frontend tests** (when applicable) verify user-visible behavior: rendered text, element visibility, interaction results — never internal state or implementation details
- Line coverage meets minimum thresholds: 80% overall, 90% for new code, 100% for critical paths
- All tests pass before any code is committed — `dotnet test` must succeed
